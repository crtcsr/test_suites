...
int generatorOptions(char **streamFile)
{
	...
	{
		option = 0; // displayGeneratorOptions();
		switch (option)
		{
		case 0:
...
void openOutputStreams(int option, int numOfBitStreams)
{
	int i, numOfOpenFiles = 0;
	char freqfn[200], summaryfn[200], statsDir[200], resultsDir[200];

	char *nist = getenv("NISTFOLDER");

	sprintf(freqfn, "nist_results/%s/%s/freq.txt", nist, generatorDir[option]);
	...
	sprintf(summaryfn, "nist_results/%s/%s/finalAnalysisReport.txt", nist, generatorDir[option]);
	...
	for (i = 1; i <= NUMOFTESTS; i++)
	{
		if (testVector[i] == 1)
		{
			sprintf(statsDir, "nist_results/%s/%s/%s/stats.txt", nist, generatorDir[option], testNames[i]);
			sprintf(resultsDir, "nist_results/%s/%s/%s/results.txt", nist, generatorDir[option], testNames[i]);
...
void invokeTestSuite(int option, char *streamFile, int BitMode)
{
	...
	{
	case 0:
		fileBasedBitStreams(streamFile, BitMode);
		break;
	case 1:
		lcg();
		break;
